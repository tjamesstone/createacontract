...
module.exports = {
    addCompany: async (req, res) => {
        ...
        const company = await db.find_company([legal_name])
        if (company[0]) return res.status(200).send(company[0])
        const {terms_of_service, logo, address, city, state, zipcode} = req.body
